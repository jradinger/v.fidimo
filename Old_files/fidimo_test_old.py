...
def midpoints_fidimo_db(fidimo_db_path):
	...
	grass.message(_("Add midpoints (vertices) to each river reach"))
	...
	# Create unique index starting with 0
	fidimo_db.execute('''ALTER TABLE vertices ADD COLUMN g_v_cat INTEGER''')
	fidimo_db.execute('''UPDATE vertices SET g_v_cat=ROWID-1''')	
	fidimo_db.execute('''CREATE INDEX g_v_index ON vertices (g_v_cat)''')
	...
	fidimo_db.execute('''ALTER TABLE edges ADD COLUMN from_g_v INTEGER''')
	fidimo_db.execute('''ALTER TABLE edges ADD COLUMN to_g_v INTEGER''')

	fidimo_db.execute('''CREATE TEMP TABLE edges_tmp (cat INTEGER, from_orig INTEGER, to_orig INTEGER)''')
	...
	fidimo_db.execute('''UPDATE edges SET 
								...
								from_g_v = (SELECT from_g_v FROM edges_tmp WHERE cat=edges.cat),
								to_g_v = (SELECT to_g_v FROM edges_tmp WHERE cat=edges.cat)
	...
	fidimo_db.execute('''CREATE INDEX edge_cat_index ON edges (cat)''')	
						
	# Get g_v (igraph related IDs) for from and to columns
	fidimo_db.execute('''UPDATE edges SET from_g_v = (SELECT g_v_cat FROM vertices 
	...
	fidimo_db.execute('''UPDATE edges SET to_g_v = (SELECT g_v_cat FROM vertices 
...
def fidimo_igraph(fidimo_db_path,network_id=None):
	...
	midpoints_fidimo_db(fidimo_db_path)
	
	
	grass.message(_("Set up main distance matrix (fidimo_main) between all connected river reaches"))


	fidimo_db.execute('''CREATE TABLE fidimo_main (from_g_v INTEGER, to_g_v INTEGER, distance DOUBLE, direction INTEGER, 
					from_orig INTEGER, to_orig INTEGER, edge_length_from DOUBLE, edge_length_to DOUBLE,
					upr_limit DOUBLE, lwr_limit DOUBLE, strahler_from INTEGER, shreve_from INTEGER, 
					shreve_to INTEGER, network INTEGER)''')

	for i in network_id:
		grass.message(_("Processing network ID: %s" %str(i)))
		# Fetch all vertices that are barriers
		#fidimo_db.execute('''SELECT g_v_cat FROM vertices WHERE v_type=1''')
		#g_vertices_barriers = [x[0] for x in fidimo_db.fetchall()]

		# Fetch all edges
		fidimo_db.execute('SELECT from_g_v, to_g_v, edge_length FROM edges WHERE network = ?', str(i))
		...
		fidimo_db.execute('SELECT g_v_cat FROM vertices WHERE v_type=3 AND orig_cat IN (SELECT cat from edges WHERE network = ? )',str(i))
		...
		fidimo_db.executemany("INSERT INTO fidimo_main (from_g_v,to_g_v,distance,direction,network) VALUES (?,?,?,?,?)",paths_to_db)
		fidimo_database.commit()

	# Create index
	#fidimo_db.execute('''CREATE INDEX main_id ON fidimo_main (from_orig,to_orig)''')
	...
	fidimo_db.execute('''UPDATE fidimo_main SET 
								from_orig = (SELECT orig_cat FROM vertices WHERE g_v_cat=fidimo_main.from_g_v)''')
	fidimo_db.execute('''UPDATE fidimo_main SET
								to_orig = (SELECT orig_cat FROM vertices WHERE g_v_cat=fidimo_main.to_g_v)''')
	# Get edge lengths for source and target reach
	fidimo_db.execute('''UPDATE fidimo_main SET 
								edge_length_from = (SELECT edge_length FROM edges WHERE cat=fidimo_main.from_orig AND part=1)''')
	fidimo_db.execute('''UPDATE fidimo_main SET
								edge_length_to = (SELECT edge_length FROM edges WHERE cat=fidimo_main.to_orig AND part=1)''')
	# Get stream order and network id for source (and target) reach
	fidimo_db.execute('''UPDATE fidimo_main SET
								strahler_from = (SELECT strahler FROM edges WHERE cat=fidimo_main.from_orig AND part=1)''')
	fidimo_db.execute('''UPDATE fidimo_main SET
								shreve_from = (SELECT shreve FROM edges WHERE cat=fidimo_main.from_orig AND part=1)''')
	fidimo_db.execute('''UPDATE fidimo_main SET
								shreve_to = (SELECT shreve FROM edges WHERE cat=fidimo_main.to_orig AND part=1)''')
	fidimo_db.execute('''UPDATE fidimo_main SET
	...
	fidimo_db.execute('''UPDATE fidimo_main SET
								lwr_limit = distance-edge_length_to/2''')
	fidimo_db.execute('''UPDATE fidimo_main SET
								upr_limit = distance+edge_length_to/2''')
...
# Get GRASS GIS network correct and ready for the fidimo database
prepare_network(input=input,
...
#import_vector(input_map=barriers, columns=barriers_columns, output_map="barriers_tmp")
#prepare_network(streams="streams_tmp", threshold=threshold)

# Create and populated fidimo database
#midpoints_fidimo_db(fidimo_db_path = '/home/radinger/Documents/FIDIMO_DB/fidimo_sqlite.db')


fidimo_igraph(fidimo_db_path = '/home/radinger/Documents/FIDIMO_DB/fidimo_sqlite.db',network_id = [2])
